...
def Tokenize(comment):
    ...
    for word in words:
        ...
        if word.isalpha():
            ...
            wordlist = re.split('[^a-zA-Z]', word)
            masterlist.extend(wordlist)
    masterlist = [element for element in masterlist if element != '' and len(element) > 1]
    return masterlist
...
def FirstLayer(net, l2_reg_val, is_training):
    ...
    #lambdaparameter = 0.0001
    lossval = l2_reg_val*net
    lossval = tf.reduce_sum(lossval)
    tf.losses.add_loss(loss=lossval, loss_collection = tf.GraphKeys.REGULARIZATION_LOSSES)
    #batch normalization
...
def EmbeddingL2RegularizationUpdate(embedding_variable, net_input, learn_rate, l2_reg_val):
    ...
    first_net = tf.nn.l2_normalize(net_input, axis =1)
    scalar_value = l2_reg_val*learn_rate*2
    #differiation of R(Y)
    result1 = tf.matmul(first_net,embedding_variable)
    #to maintain dimensionality transpose results in [40,2] matrix
    transpose_matrix = tf.transpose(first_net)
    #result2 will be a [40,2]*[2*40] = [40,40]
    result2 = tf.matmul(transpose_matrix, result1)
    #resultt = tf.nn.l2_normalize(result2,axis = 1)
    result3 = tf.multiply(scalar_value,result2)
    #do the update (y = y-learn_rate*(result2))
    embedding_variable1 = embedding_variable - result3
    
    return embedding_variable1 
...
def EmbeddingL1RegularizationUpdate(embedding_variable, net_input, learn_rate, l1_reg_val):
    """Accepts tf.Variable, tensor (batch_size, vocab size), regularization coef.
    Returns tf op that applies one regularization step on embedding_variable."""
    #print(embedding_variable)
    #print(net_input)
    # TODO(student): Change this to something useful. Currently, this is a no-op.
    first_net = tf.nn.l2_normalize(net_input,axis = 1)
    transpose_input = tf.transpose(first_net)
    #creating[2*40] matrix
    result1 = tf.matmul(net_input, embedding_variable)
    #creating [40,2]*[2*40]=[40*40]matrix by multiplying with transpose of first net
    #l1 norm for (d(R(Y)/DY) = x/|x|
    l1_norm = tf.sign(result1)
    ryfunction = (tf.matmul(transpose_input,l1_norm))
    scalar_value = l1_reg_val * learn_rate
    result2 = tf.multiply(scalar_value,ryfunction)
    #result3 = scalar_value*result2
    embedding_variable2 = embedding_variable - result2
    return embedding_variable.assign(embedding_variable2)
...
#EMBEDDING_VAR = None #remove this 
...
def ComputeTSNE(embedding_matrix):
    ...
    """
    #uncomment the previous lines to generate the embeddings file
    #embeds_2d = TSNE(learning_rate=50, n_iter=500).fit_transform(embedding_matrix)
    #np.savetxt('embeddingmatrix.txt', embeds_2d, delimiter=',', fmt = '%f')
...
def VisualizeTSNE(sess):
    ...
    tsne_embeddings = np.loadtxt('embeddingmatrix.txt', delimiter=',', dtype=float)
    class_to_words = {
        'positive': [
    ...
    getdata = []
    #getting the term index, please uncommment the below line to get the termindex dictionary
    #termIndexDict = json.load(open("termindex.txt"))
    ...
    ylocation = []
    counter = 0
    print(len(termIndexDict))
    dictkeys = list(termIndexDict.keys())

    for word in class_to_words['positive']:
        positiveClassIndex.append(termIndexDict[word])
    for index in positiveClassIndex:
        xpositive.append(tsne_embeddings[index][0])
        ypositive.append(tsne_embeddings[index][1])
    for word in class_to_words['negative']:
        negativeClassIndex.append(termIndexDict[word])
    for index in negativeClassIndex:
        xnegative.append(tsne_embeddings[index][0])
        ynegative.append(tsne_embeddings[index][1])
    for word in class_to_words['furniture']:
        furnitureClassIndex.append(termIndexDict[word])
    for index in furnitureClassIndex:
        xfurniture.append(tsne_embeddings[index][0])
        yfurniture.append(tsne_embeddings[index][1])
    for word in class_to_words['location']:
        locationClassIndex.append(termIndexDict[word])
        
    for index in locationClassIndex:
        xlocation.append(tsne_embeddings[index][0])
        ylocation.append(tsne_embeddings[index][1]) 
    
    colorlist = ['blue','orange','red','green']
    for i in range(len(positiveClassIndex)):
        plt.scatter(xpositive[i], ypositive[i],color = 'blue')
        plt.annotate(class_to_words['positive'][i], xy=(xpositive[i], ypositive[i]))
        print(class_to_words['positive'][i], xpositive[i], ypositive[i])

    for i in range(len(negativeClassIndex)):
        plt.scatter(xnegative[i], ynegative[i], color='orange')
        plt.annotate(class_to_words['negative'][i], xy=(xnegative[i], ynegative[i]))
        print(class_to_words['negative'][i], xnegative[i], ynegative[i])

    for i in range(len(furnitureClassIndex)):
        plt.scatter(xfurniture[i], yfurniture[i], color='red')
        plt.annotate(class_to_words['furniture'][i], xy=(xfurniture[i], yfurniture[i]))
        print(class_to_words['furniture'][i], xfurniture[i], yfurniture[i])

    for i in range(len(locationClassIndex)):
        plt.scatter(xlocation[i], ylocation[i], color='green')
        plt.annotate(class_to_words['location'][i], xy=(xlocation[i], ylocation[i]))
        print(class_to_words['location'][i], xlocation[i], ylocation[i])
